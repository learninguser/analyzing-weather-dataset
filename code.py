...
def clear(df,col,val):
    ...
    """
    value_counts = df[df[col] == val].count()[col]
    return value_counts
...
def convert(df,celsius):
    ...
    converted_temp =  9/5 *centigrade_temps + 32
...
categorical_var, numerical_var = categorical(weather), numerical(weather)
...
cloudy = clear(weather, 'Weather', 'Cloudy')
